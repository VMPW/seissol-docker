...
RUN apt-get update -qq && apt-get install wget software-properties-common -y --no-install-recommends && add-apt-repository -y ppa:ubuntu-toolchain-r/test && apt-get update -qq && apt-get install -y -qq g++-5 gfortran openmpi-bin openmpi-common libopenmpi-dev hdf5-tools libhdf5-openmpi-10 libhdf5-openmpi-dev python3 python3-pip libmetis-dev libparmetis-dev m4 unzip git python cmake pkg-config --no-install-recommends && rm -rf /var/lib/apt/lists/*
RUN pip3 install --upgrade pip && pip3 install 'numpy>=1.12.0' lxml
